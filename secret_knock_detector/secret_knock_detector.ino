...
/* Detects patterns of pushes and triggers a motor to unlock
   ...
   Digital Pin 6: IN Switch to enter a new code.  Short this to +5VDC enter programming mode.
   Digital Pin 8: OUT Relay to open door connected here. (Or a motor controller or a solenoid or other unlocking mechanisim.)
   Digital Pin 4: OUT LED1. 
   Digital Pin 5: OUT LED2. 
   Digital pin 9: IN doorBel switch.
   Digtial pin 7: OUT Relay for doorbell.
...
const int LED1 = 5;              // Status LED
const int LED2 = 4;            // Status LED
...
void setup() {
  ...
  digitalWrite(LED2, HIGH);      // Green LED on, everything is go.
}
...
void loop() {
  ...
  if (digitalRead(programSwitch)==HIGH){  // is the program button pressed?
    ...
    digitalWrite(LED1, HIGH);           // and turn on the red light too so we know we're programming.
  } else {
...
void listenToSecretPush(){
  ...
  digitalWrite(LED2, LOW);      			// we blink the LED for a bit as a visual indicator of the push.
  if (programButtonPressed==true){
     digitalWrite(LED1, LOW);                         // and the red one too if we're programming a new push.
  }
  ...
  do {
    ...
    if (doorbellValue == HIGH){                   //got another push...
      ...
      if (programButtonPressed==true){
        digitalWrite(LED1, LOW);                       // and the red one too if we're programming a new code.
      }
  ...
  if (programButtonPressed==false){             // only if we're not in progrmaing mode.
    if (validatePush() == true){
      ...
      ringDoorbell();
      digitalWrite(LED2, LOW);  		// We didn't unlock, so blink the red LED as visual feedback.
      for (i=0;i<4;i++){					
        digitalWrite(LED1, HIGH);
        delay(100);
        digitalWrite(LED1, LOW);
...
void triggerDoorUnlock(){
  ...
  digitalWrite(LED2, HIGH);            // And the green LED too.
